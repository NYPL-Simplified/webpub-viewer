  ...
  describe("#enable", () => {
    ...
    it("should cache a manifest that's not in the cache yet", async () => {
      ...
      const manifest = new Manifest(
        {
          spine: [{ href: "spine-item-1.html" }, { href: "spine-item-2.html" }],
          resources: [{ href: "resource-1.html" }, { href: "resource-2.html" }]
        },
      ...
      expect(urlsThatWereCached).to.contain(
        "https://example.com/manifest.json"
      );
      expect(urlsThatWereCached).to.contain(
        "https://example.com/static-1.html"
      );
      expect(urlsThatWereCached).to.contain(
        "https://example.com/static-2.html"
      );
      expect(urlsThatWereCached).to.contain(
        "https://example.com/spine-item-1.html"
      );
      expect(urlsThatWereCached).to.contain(
        "https://example.com/spine-item-2.html"
      );
      expect(urlsThatWereCached).to.contain(
        "https://example.com/resource-1.html"
      );
      expect(urlsThatWereCached).to.contain(
        "https://example.com/resource-2.html"
      );
    });
  });
    ...
    it("should provide status updates when there's an error", async () => {
      ...
      open.returns(
        new Promise<void>((_, reject) =>
          reject(new Error("Status failure"))
        ).catch(error => {
