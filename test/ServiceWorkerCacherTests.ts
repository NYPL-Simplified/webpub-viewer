...
describe('ServiceWorkerCacher', () => {
    ...
    describe('#renderStatus', () => {
        ...
        it("should fall back to application cache if the Cache API is not supported and fallback URL is provided", async () => {
            ...
            const appCacheStub = stub(ApplicationCacheCacher.prototype, "renderStatus");
            ...
            await cacher.renderStatus(element);
            expect(register.callCount).to.equal(0);
            expect(appCacheStub.callCount).to.equal(1);
            expect(appCacheStub.args[0][0]).to.equal(element);

            appCacheStub.restore();
        });
