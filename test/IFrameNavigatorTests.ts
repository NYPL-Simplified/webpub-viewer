...
describe("IFrameNavigator", () => {
    ...
    const window =
      jsdom.jsdom("", {
        ...
        resourceLoader: (resource: any, callback: any) => {
          ...
          callback(
            null,
            "<html><body>Some HTML for " + resource.url.href + "</body></html>"
          );
        },
        features: {
  ...
  describe("#start", () => {
    ...
    it("should render the settings controls", async () => {
      ...
      const settingsView = element.querySelector(
        ".settings-view"
      ) as HTMLDivElement;
    ...
    it("should give the settings a function to call when the font size changes", async () => {
      ...
      await pause();
      expect((iframe.contentDocument as any).body.style.fontSize).to.equal(
        "14px"
      );
      expect((iframe.contentDocument as any).body.style.lineHeight).to.equal(
        "1.5"
      );
    ...
    it("should go into fullscreen mode and exit it", async () => {
      ...
      expect(fsmIcon.getAttribute("aria-labelledby")).to.contain(
        "minimize-icon"
      );
    ...
    it("should show the navigation links in paginated view", async () => {
      ...
      const linksBottom = element.querySelector(
        "ul.links.bottom"
      ) as HTMLUListElement;
    ...
    it("should set heights on view and top and bottom info when loading iframe", async () => {
      const iframe = element.querySelector("iframe") as HTMLIFrameElement;
      const linksTop = element.querySelector(
        "ul.links.top"
      ) as HTMLUListElement;
      const linksBottom = element.querySelector(
        "ul.links.bottom"
      ...
      const infoBottom = element.querySelector(
        ".info.bottom"
      ) as HTMLDivElement;
    ...
    it("should inject the readonly epubReadingSystem object into the iframe’s window.navigator", async () => {
      ...
      try {
        ((iframe.contentWindow as any)
          .navigator as any).epubReadingSystem.name = "";
      } catch (_err) {
        err = _err;
      }
  ...
  describe("table of contents", () => {
    ...
    it("should handle toc entries which don’t have an href", async () => {
      ...
      const sublinks = (spans[0].parentElement as any).querySelectorAll(
        "ol > li"
      );
      expect(sublinks.length).to.equal(2);
    ...
    it("should show and hide when contents control is clicked", async () => {
      ...
      const openIcon = contentsControl.querySelector(
        ".icon.open"
      ) as SVGElement;
      const closeIcon = contentsControl.querySelector(
        ".icon.close"
      ) as SVGElement;
      getSelectedView.returns(scroller);
    ...
    it("should hide when other navigation links are clicked", async () => {
      ...
      const nextChapterLink = element.querySelector(
        "a[rel=next]"
      ) as HTMLAnchorElement;
      ...
      const previousChapterLink = element.querySelector(
        "a[rel=prev]"
      ) as HTMLAnchorElement;
    ...
    it("should hide rest of page from screen readers when open", () => {
      ...
      for (const button of Array.prototype.slice.call(
        topNav.querySelectorAll(".controls > button, .controls > li button")
      )) {
      ...
      for (const link of Array.prototype.slice.call(
        topNav.querySelectorAll(".controls > a, .controls > li a")
      )) {
        expect(link.getAttribute("aria-hidden")).to.equal("true");
      }

      // // Every controls view except the toc should be hidden.
      for (const view of Array.prototype.slice.call(
        topNav.querySelectorAll(".controls-view")
      )) {
        if (view !== toc) {
      ...
      for (const bottomLinks of Array.prototype.slice.call(
        bottomNav.querySelectorAll(".links")
      )) {
  ...
  describe("settings", () => {
    it("should show and hide when settings control is clicked", async () => {
      const iframe = element.querySelector("iframe") as HTMLIFrameElement;
      const settings = element.querySelector(
        ".settings-view"
      ) as HTMLDivElement;
      const settingsControl = element.querySelector(
        "button.settings"
      ) as HTMLButtonElement;
      const openIcon = settingsControl.querySelector(
        ".icon.open"
      ) as SVGElement;
      const closeIcon = settingsControl.querySelector(
        ".icon.close"
      ) as SVGElement;
