...
export default class BookSettings {
  ...
  private static readonly SELECTED_FONT_SIZE_KEY =
    "settings-selected-font-size";
  ...
  public static async create(config: BookSettingsConfig) {
    const fontSizes = config.fontSizesInPixels.map(
  ...
  private async initializeSelections(defaultFontSize?: string): Promise<void> {
    if (this.bookFonts.length >= 1) {
      ...
      const selectedFontName = await this.store.get(
        BookSettings.SELECTED_FONT_KEY
      );
      if (selectedFontName) {
    ...
    if (this.fontSizes.length >= 1) {
      ...
      let selectedFontSize = await this.store.get(
        BookSettings.SELECTED_FONT_SIZE_KEY
      );
      let selectedFontSizeIsAvailable =
        selectedFontSize && this.fontSizes.indexOf(selectedFontSize) !== -1;
    ...
    if (this.bookThemes.length >= 1) {
      ...
      const selectedThemeName = await this.store.get(
        BookSettings.SELECTED_THEME_KEY
      );
    ...
    if (this.bookViews.length >= 1) {
      ...
      const selectedViewName = await this.store.get(
        BookSettings.SELECTED_VIEW_KEY
      );
      if (selectedViewName) {
  ...
  public renderControls(element: HTMLElement): void {
    ...
    if (this.fontSizes.length > 1) {
      for (const fontSizeName of ["decrease", "increase"]) {
        this.fontSizeButtons[fontSizeName] = HTMLUtilities.findRequiredElement(
          element,
  ...
  private setupEvents(): void {
    ...
    if (this.fontSizes.length > 1) {
      this.fontSizeButtons["decrease"].addEventListener(
        "click",
        (event: MouseEvent) => {
      ...
      this.fontSizeButtons["increase"].addEventListener(
        "click",
        (event: MouseEvent) => {
          const currentFontSizeIndex = this.fontSizes.indexOf(
            this.selectedFontSize
